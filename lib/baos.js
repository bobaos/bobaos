...
class Baos extends EventEmitter {
  ...
  // serial port events
  _ft12onOpen() {
    debug('', 'ft12 port opened!');
    this.emit('open');
  }

  _ft12onAck(ack) {
    debug('ft12 on ack: ', ack);
    this.emit('ack', ack);
  }

  _ft12onReset() {
    debug('ft12 on reset ind');
    this.emit('reset');
    this.ft12.sendAck();
  }
          ...
          if (datapoint) {
            ...
            switch (type) {
              ...
              case 'dpt9':
                ...
                break;
              default:
                error('datapoint type is not supported', type);
                break;
            }
            ...
            error('cannot find datapoint by id: ', payloadItem);
          }
        });
      } else {
        error('payload is not array!', payload);
      }
    };
    if (data && data.payload) {
      ...
      switch (service) {
        ...
        case 'GetDatapointValue.Res':
          ...
          break;
        default:
          error('service is not implemented yet:', service);
      }
    }
    ...
    if (datapointItem) {
      ...
      switch (datapointItem.type) {
        ...
        case 'dpt9':
          ...
          break;
        default:
          error('datapoint type isn\'t supported: ', datapointItem);
          break;
      }
      ...
      debug('setDatapoint we are sending data', message);
      // now we send message
      this.ft12.sendMessage(message);
    } else {
      error('cannot find datapoint by name: ', datapointName);
    }
  }
  ...
  setDatapointById(datapointId, setValue) {
    ...
    if (datapointItem) {
      ...
      error('cannot find datapoint by id: ', datapointId);
    }
  }
  ...
  readDatapoint(datapointName) {
    ...
    if (datapointItem) {
      ...
      debug('read datapoint', message.map(item => item.toString(16)));
      // now we send message
      this.ft12.sendMessage(message);
    } else {
      error('cannot find datapoint by name: ', datapointName);
  ...
  getDescriptionString(start, number) {
    ...
    debug('get descr string', message.map(item => item.toString(16)));
    this.ft12.sendMessage(message);
  }
