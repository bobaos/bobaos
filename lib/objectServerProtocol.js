...
// TODO: logger
// TODO: datapoint number in two bytes
...
class ObjectServerProtocol {
        ...
        if (service) {
            ...
            if (payload) {
                ...
                if (Array.isArray(payload)) {
                    ...
                    //   let payloadItem = payloadItemCmd(item);
                    //   message = Buffer.concat([message, payloadItem]);
                    // })
                } else {
                ...
                logger.info('please specify payload');
            }
        } else {
            logger.info('unknown service');
        }
    ...
    static __bufferToNumber(buf) {
        let byte1 = buf[0];
        let byte0 = buf[1];
        // console.log('__buffToNumber', buf, (byte1 << 8) | byte0);
    ...
    static parseMessage(data) {
        logger.info('ObjServerProto parse data:', data);
        const parseDatapoints = (dtps) => {
            ...
            logger.info('parse message: ', returnData);
        ...
        if (Buffer.isBuffer(data) || Array.isArray(data)) {
            ...
            if (service) {
                ...
                logger.info("service is not found")
            }
        } else {
            logger.info("data is not buffer or array")
        }
    }
}
